...
* **Performance:** Implements lazy loading for history records and enforces `LIMIT 200` to handle large data volumes.

### 2. Automation & Audit
* **Trigger:** `BookingTrigger` (After Update)
    * Automatically creates immutable records in `Booking_History__c` whenever a Booking's Status or Date is changed.
* **Batch Apex:** `DailyDeliveryNotificationBatch`
...
* **Permissions:** Custom Permission Set (`Booking_Portal_Access`) for granular object access.
